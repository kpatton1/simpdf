...
for i in range(0,5):
    data = numpy.load('data_real/F_1024_test_pdf256_' + str(i) + '.npz')
    bins = 200.0 * numpy.power(2.0,i)
    F = data['F']
...
for i in range(0,4):
    data = numpy.load('data_real/F_1024_test_pdf256_2_' + str(i) + '.npz')
    bins = 300.0 * numpy.power(2.0,i)
    F = data['F']
    #F = (N-bins-2.0)/(N-2.0) * F
    F += prior
    fom = calc_fisher_fom(F)
...
plt.title('PDF 256 resolution scaling')
