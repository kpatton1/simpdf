...
fisher = 'data_real/F_1024_test_pdf1024_2_2.npz'
basetitle = 'PDF 1024 scaling'

fisher2 = 'data_real/F_1024_test_pdf1024_3_0.npz'

fisher3 = 'data_real/F_1024_test_pdf1024_3_4.npz'

fisher4 = 'data_real/F_1024_test_pdf1024_4_0.npz'

fisher5 = 'data_real/F_1024_test_pdf1024_4_1.npz'
...
F = data['F'] #* (N - 1000.0 - 2.0) / (N - 2.0)
params = data['params']

data = numpy.load(fisher2)
F2 = data['F'] #* (N - 2000.0 - 2.0) / (N - 2.0)

data = numpy.load(fisher3)
F3 = data['F'] #* (N - 3000.0 - 2.0) / (N - 2.0)

data = numpy.load(fisher4)
F4 = data['F'] #* (N - 4000.0 - 2.0) / (N - 2.0)

data = numpy.load(fisher5)
F5 = data['F'] #* (N - 5000.0 - 2.0) / (N - 2.0)
...
for i in range(n):
    for j in range(i):
        ...
        plt.plot([],color='blue',label='1000 bins')

        plot_fisher(F2, basetitle, i, j, 'green',True)
        plt.plot([],color='green',label='2000 bins')

        plot_fisher(F3, basetitle, i, j, 'red', False)
        plt.plot([],color='red',label='3000 bins')
        
        plot_fisher(F4,basetitle,i,j, 'black',False)
        plt.plot([],color='black',label='4000 bins')

        plot_fisher(F5, basetitle, i, j, 'purple', False)
        plt.plot([],color='purple',label='5000 bins')
